...
export default function AccountsPage() {
	...
	async function createAccount(event: React.FormEvent<HTMLFormElement>) {
		...
		try {
			...
			let normalizedUsername = submittedUsername;
			let normalizedDomainId = domainId;
			if (atIndex !== -1) {
				const hostname = submittedUsername.slice(atIndex + 1).toLowerCase();
				const matchingDomain = domains.find((domain) => domain.hostname.toLowerCase() === hostname);
				if (!matchingDomain) throw new Error("That email domain is not connected yet");
				normalizedUsername = submittedUsername.slice(0, atIndex);
				normalizedDomainId = matchingDomain.id;
			}
			const response = await authFetch("/api/accounts", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ username: normalizedUsername, domainId: normalizedDomainId, password, role }) });
			const data = (await response.json()) as AccountResponse;
	...
	return <div className="space-y-6">
		<div className="flex items-center justify-between gap-4"><div><h1 className="text-3xl font-medium text-neutral-900">Accounts</h1><p className="mt-2 text-sm text-neutral-500">Manage user accounts and their inboxes.</p></div>{!teamRequired && <Button onClick={() => setCreateOpen(true)}><Plus className="h-4 w-4" />New account</Button>}</div>
		...
		<Dialog open={createOpen} onOpenChange={setCreateOpen}><DialogContent><DialogHeader><DialogTitle>Add user account</DialogTitle><DialogDescription>The user can sign in with this email and password.</DialogDescription></DialogHeader><form onSubmit={createAccount} className="space-y-4">
			<div className="space-y-2"><Label htmlFor="account-username">Email</Label><div className="flex h-10 overflow-hidden rounded-md border border-neutral-200 bg-white"><Input id="account-username" value={username} onChange={(event) => setUsername(event.target.value)} placeholder="username or full email address" className="min-w-0 flex-1 rounded-none border-0 shadow-none" required />{!username.includes("@") && <><span className="flex items-center text-sm text-neutral-400">@</span><Select aria-label="Domain" value={domainId} onChange={(event) => setDomainId(event.target.value)} className="max-w-[55%] bg-transparent px-3 text-sm" required><option value="">Select domain</option>{domains.map((domain) => <option key={domain.id} value={domain.id}>{domain.hostname}</option>)}</Select></>}</div><p className="text-xs text-neutral-500">Enter the username and choose a domain, or paste the complete email address.</p></div>
